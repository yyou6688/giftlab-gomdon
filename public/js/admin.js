...
async function login(){
  ...
  try{
    ...
    errEl.textContent = 'Không kết nối được tới server, thử lại sau vài giây (server có thể đang khởi động lại).';
  } finally {
    btn.disabled = false;
...
function renderCategoryManageRows(){
  return categories.map((c, i) => {
    ...
    return `
      <div class="category-row" draggable="true" data-cat-key="${escapeHtml(c.key)}" style="padding:8px 0; border-bottom:1px dashed var(--line);">
        <div style="display:flex; align-items:center; gap:10px;">
          <span class="drag-handle" title="Giữ chuột và kéo để đổi vị trí" style="cursor:grab; font-size:16px; color:var(--ink-soft); user-select:none;">⠿</span>
...
// thư viện ngoài) - gắn 1 lần duy nhất bằng event delegation, chỉ nhận kéo bắt đầu từ
// đúng dòng danh mục (.category-row), giữ nguyên mũi tên lên/xuống làm phương án dự
// phòng khi kéo-thả không tiện dùng (VD: đang mở trên điện thoại/tablet)
let dragCategoryKey = null;
document.addEventListener('dragstart', (e) => {
  const row = e.target.closest && e.target.closest('.category-row');
  if(!row) return;
  dragCategoryKey = row.dataset.catKey;
  row.style.opacity = '0.4';
});
document.addEventListener('dragend', (e) => {
  const row = e.target.closest && e.target.closest('.category-row');
  if(row) row.style.opacity = '1';
  dragCategoryKey = null;
});
document.addEventListener('dragover', (e) => {
  if(e.target.closest && e.target.closest('.category-row')) e.preventDefault();
});
document.addEventListener('drop', async (e) => {
  const row = e.target.closest && e.target.closest('.category-row');
  if(!row || !dragCategoryKey) return;
  e.preventDefault();
  const dropKey = row.dataset.catKey;
  if(dropKey === dragCategoryKey) return;
  const fromIdx = categories.findIndex(c => c.key === dragCategoryKey);
  const toIdx = categories.findIndex(c => c.key === dropKey);
